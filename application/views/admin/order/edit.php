        ...
        success: function(data) {
            ...
            $.ajax({
                ...
                success: function(datax) {
                    ...
                    if (datax == null) {
                        Subtotal      = 0;
                        Ppn_total     = formatter1.format(Ppn_total);
                        Ppn_rp_total  = 0;
                        Total         = 0;
                    } else {
                        Subtotal      = formatter.format(datax.subtotal);
                        Ppn_total     = formatter1.format(Ppn_total);
                        Ppn_rp_total  = ((Ppn_total*datax.subtotal)/100);
                        Total         = parseInt(datax.subtotal)+parseInt(Ppn_rp_total);
                        if (isNaN(Total)) {
                            Total         = 0;
                        } else {
                    ...
                    $('#ppn_modal').val(Ppn_total);
                    $('#ppn_rp_modal').val(Ppn_rp_total);
...
function hitungDiskon() {
    ...
    var Subtotal            = myForm.order_subtotal.value;
    Subtotal                = Subtotal.replace(/[,]/g, '');
    Subtotal                = parseInt(Subtotal);
    var Diskon              = myForm.diskon.value;
    Diskon                  = Diskon.replace(/[,]/g, '');
    Diskon                  = parseInt(Diskon);
    var Ppn                 = myForm.order_ppn.value;
    Ppn                     = Ppn.replace(/[,]/g, '');
    Ppn                     = parseFloat(Ppn);
    var Total               = myForm.order_total.value;
    Total                   = Total.replace(/[,]/g, '');
    Total                   = parseInt(Total);

    if (Diskon === 0 || isNaN(Diskon)) {
        var SubTotalAkhir   = Subtotal;
        var PPN             = ((Ppn*SubTotalAkhir)/100);
    ...
    if (isNaN(TotalAkhir)) {
        myForm.order_total.value = 0;
    } else {
        myForm.order_total.value = formatter.format(TotalAkhir);
    }
}
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // resetformBayar();
                    // window.location="<?=site_url('admin/order/adddata');?>";
                },
                error: function() {
                    swal({
                        title:"Error",
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <input type="hidden" name="ppn_rp_modal" id="ppn_rp_modal">
            </div>
            <div class="modal-body">
                <div class="form-group form-md-line-input">
                ...
                <div class="form-group form-md-line-input">
                    ...
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="order_subtotal" id="subtotal_modal" autocomplete="off" readonly>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group form-md-line-input">
                    ...
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="order_ppn" id="ppn_modal" autocomplete="off" readonly>
                    </div>
                </div>
                <div class="form-group form-md-line-input">
