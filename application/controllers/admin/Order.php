...
class Order extends MY_Controller
{
    ...
    {
        $data = $this->db->get_where('resto_order', array('order_id' => $order_id))->row();
        echo json_encode($data);
    }
    ...
    public function bayar()
    {
        ...
        $data       = array(
            'order_qty'       => $qty,
            'order_subtotal'  => intval(str_replace(",", "", $this->input->post('order_subtotal', 'true'))),
            'order_diskon'    => intval(str_replace(",", "", $this->input->post('diskon', 'true'))),
            'order_ppn'       => str_replace(",", "", $this->input->post('order_ppn', 'true')),
            'order_ppn_rp'    => $this->input->post('ppn_rp_modal', 'true'),
            'order_total'     => intval(str_replace(",", "", $this->input->post('order_total', 'true'))),
    ...
    public function cetaknotabayar($order_id)
    {
        ...
        $dataOrder   = $this->db->get_where('resto_order', array('order_id' => $order_id))->row();
        $listItem    = $this->db->get_where('resto_order_detail', array('order_id' => $order_id))->result();
        ...
        foreach ($listItem as $r) {
            ...
            $Disc     = number_format($r->order_detail_disc, 2, '.', ',');
            $Subtotal = number_format($r->order_detail_subtotal, 0, '', ',');
            $Data .= $this->addItem($NamaMenu, $Harga, $Qty, $Disc, $Subtotal);
        }
        ...
        $Data .= $this->addFooter($SubTotal, $Diskon, $Pajak, $Total, $NPWPD);
        fwrite($handle, $Data);
        fclose($handle);
    ...
    public function addHeader($NamaToko, $AlamatToko, $TelpToko, $NoOrder, $Tanggal, $Kasir)
    {
        ...
        $returnValue .= "========================================" . chr(10);
        $returnValue .= "Menu            Harga Qty Disc  Subtotal" . chr(10);
        $returnValue .= "========================================" . chr(10);
        return $returnValue;
    }

    public function addItem($NamaMenu, $Harga, $Qty, $Disc, $Subtotal)
    {
        // LimitCharacter
        $limitNamaMenu = 13;
        $limitHarga    = 7;
        ...
        $returnValue = "" . $txtNamaMenu . " " . $txtHarga . " " . $txtQty . " " . $txtDisc . " " . $txtSubtotal . chr(10);
        return $returnValue;
    }
